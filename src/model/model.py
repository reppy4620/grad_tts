...
class TTSModel(nn.Module):
    ...
    def compute_loss(self, batch):
        ...
        off_set_ranges = list(zip([0] * len(y_lengths), (y_lengths - self.out_size).clamp(0).cpu().numpy()))
        out_offset = torch.LongTensor([
            torch.tensor(random.choice(range(start, end)) if end > start else 0)
            for start, end in off_set_ranges
        ]).to(y_lengths)
        shape = (x.shape[0], x.shape[1], self.out_size)
        m_cut = torch.zeros(*shape).to(m)
        y_cut = torch.zeros(*shape).to(y)
        y_mask_cut = torch.zeros(x.shape[0], 1, self.out_size).to(y_mask)
        for i, offset in enumerate(out_offset):
            cut_length = self.out_size + (y_lengths[i] - self.out_size).clamp(None, 0)
